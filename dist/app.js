...
const dorszopediaEntries = [
  ...
  { id: 'historie', icon: '📚', category: 'Bohaterowie', title: 'Jak czytać opowiadania Dorszo-Kumpli', intro: 'Każda ukończona historia ma bohaterów, zagadkę, decyzję i zakończenie.', body: ['Biblioteka zawiera pięć pełnych, ukończonych opowieści: o Złotym Pęcherzu, znikających plecakach, programie „Ryba z Misją”, Algorii i wirtualnej mapie miasta. Każdy tytuł jest podzielony na cztery rozdziały, więc można wrócić do czytania w dogodnym momencie.', 'Historie uczą przez przygodę, a nie przez straszenie. Nawet gdy bohaterowie się mylą, mogą porozmawiać, przeprosić, poprawić plan i wrócić do wspólnego działania.'], facts: ['Wszystkie historie mają zakończenie.', 'Dorszuś i Borys są częścią miasta Króla Dorsza.'] },
...
function showCityStories() {
  openModal(`<article class="story-modal"><p class="modal-eyebrow">Dorszo-Kumple · ukończone historie</p><h2>Biblioteka Dorszolandii</h2><p>Dorszuś i Borys są częścią dzisiejszego miasta Króla Dorsza. Każda opowieść ma zakończenie, ważną rozmowę i odrobinę bąbelkowego chaosu.</p><div class="story-library">${storyLibrary.map(story => `<button type="button" class="story-card" data-story-open="${story.id}"><span class="story-card-icon" aria-hidden="true">${story.icon}</span><span><span class="story-status is-complete">Ukończona · ${story.minutes} min czytania</span><strong>${story.title}</strong><small>${story.teaser}</small></span><b aria-hidden="true">→</b></button>`).join('')}</div><div class="fact-box"><strong>W kronice:</strong> wszystkie ${storyLibrary.length} opowieści są gotowe do przeczytania od pierwszego do ostatniego rozdziału.</div></article>`);
}

function openCityStory(id) {
  const story = storyLibrary.find(item => item.id === id);
  if (!story) return;
  openModal(`<article class="story-modal"><button type="button" class="story-back" data-stories-home="true">← Wszystkie historie</button><p class="modal-eyebrow">Dorszo-Kumple · pełna opowieść</p><p class="story-status is-complete">Ukończona historia · około ${story.minutes} min czytania</p><h2>${story.icon} ${story.title}</h2><p>${story.teaser}</p><div class="modal-people">${story.people.map(person => `<span class="modal-person">${person}</span>`).join('')}</div><div class="story-reader">${story.chapters.map((chapter, index) => `<section class="story-chapter"><span class="story-chapter-number">Rozdział ${index + 1}</span><h3>${chapter.title}</h3>${chapter.paragraphs.map(paragraph => `<p>${paragraph}</p>`).join('')}</section>`).join('')}</div><div class="mission-box"><strong>Po lekturze:</strong> Która decyzja bohaterów była najmądrzejsza? Jak Ty pomógłbyś mieszkańcom Dorszolandii?</div></article>`);
}

function storyShelfCard(story) {
  return `<button type="button" class="story-shelf-card" data-story-open="${story.id}" aria-label="Czytaj opowiadanie: ${story.title}"><img src="assets/${story.cover}" alt="" loading="lazy" /><span class="story-shelf-copy"><span class="story-status is-complete">Ukończona · ${story.minutes} min</span><strong>${story.title}</strong><small>${story.teaser}</small><em>Czytaj pełną opowieść →</em></span></button>`;
}

function renderStoryShelf() {
  const shelf = $('#storyShelf');
  shelf.innerHTML = storyLibrary.map(storyShelfCard).join('');
