...
const canonicalStories = [...archiveStories, ...existingStories].sort((a, b) => a.number - b.number);
...
const canonicalProfiles = {
  ...
  'babel-maksymalny': {
    ...
    description: 'Wierny kompan Borysa i Dorszusia oraz specjalista od pojawiania się w najmniej przewidywalnym momencie.'
  }
};
...
function addLibraryStyles() {
  ...
  style.textContent = `
    ...
    .story-section-note{margin:1rem 0 1.35rem;padding:1rem 1.1rem;border-radius:18px;background:rgba(255,255,255,.78);box-shadow:0 10px 30px rgba(19,31,68,.08)}
    .story-section-note b{display:block;font-size:1.05rem;margin-bottom:.25rem}
    .story-section-note p{margin:0;line-height:1.55}
    .story-cover-fallback{width:100%;aspect-ratio:16/10;display:grid;place-items:center;font-size:4rem;background:linear-gradient(145deg,#e9f8ff,#f3e9ff);border-radius:inherit}
...
function openCanonicalStory(story) {
  ...
  modalContent.innerHTML = `
    <article class="reading-modal story-reading">
      <p class="kicker">${section.title} · opowiadanie ${story.number} z 40</p>
      <h2>${escapeHtml(story.title)}</h2>
      <p class="story-meta">${story.minutes} minut czytania · pełny tekst z wydania kompletnego</p>
      <section class="story-chapter original-text">${content}</section>
...
function upgradeLibrary() {
  ...
  if (title) title.textContent = '40 pełnych opowiadań';
  const bannerCopy = banner.querySelector('p:not(.kicker)');
  if (bannerCopy) bannerCopy.textContent = 'Wydanie kompletne: 40 historii w trzech częściach. Jeden tytuł to zawsze jedno pełne opowiadanie — bez skracania i bez sztucznego dzielenia.';
  if (!banner.querySelector('.library-canonical-summary')) {
    const summary = document.createElement('div');
    summary.className = 'library-canonical-summary';
    summary.innerHTML = '<span>40 historii</span><span>3 części</span><span>1 tytuł = 1 pełne opowiadanie</span>';
  ...
  const render = () => {
    ...
    shelf.innerHTML = `<article class="story-section-note"><b>${meta.title} • ${escapeHtml(meta.subtitle)}</b><p>${escapeHtml(meta.description)}</p></article>` + stories.map(story => {
      ...
      return `<button class="story-card" type="button" data-canon-story-id="${escapeHtml(story.id)}">
        ${artwork}
        <span><small><span class="story-card-number">NR ${story.number}/40</span> · ${story.minutes} min</small><strong>${escapeHtml(story.title)}</strong><em>${escapeHtml(story.teaser)}</em><b>Czytaj pełne opowiadanie →</b></span>
      </button>`;
    }).join('');
  };
  ...
  console.info(`[Dorszolandia] Biblioteka kanoniczna: ${canonicalStories.length} pełnych opowiadań (20 + 5 + 15).`);
}
