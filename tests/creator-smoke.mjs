...
try {
  await page.goto('http://127.0.0.1:4173/kreator.html', { waitUntil: 'domcontentloaded', timeout: 20000 });
  ...
  await page.$eval('#accessoryRotation', el => { el.value = '45'; el.dispatchEvent(new Event('input', { bubbles: true })); });
  ...
  await page.$eval('#fishName', el => { el.value = 'Testowy Dorsz'; el.dispatchEvent(new Event('input', { bubbles: true })); });
  ...
  const mobile = await page.evaluate(() => ({ scrollWidth: document.documentElement.scrollWidth, clientWidth: document.documentElement.clientWidth, columns: getComputedStyle(document.querySelector('#kreator .creator-premium-layout')).gridTemplateColumns, stageWidth: document.querySelector('#creatorStage').getBoundingClientRect().width }));
